...
def assess_v2(ad_text, masc_w=wordlists.masculine_coded_words, fem_w=wordlists.feminine_coded_words):
    ...
    masc_words = [adword for adword in ad_text
        for word in masc_w
        if adword.startswith(word)]
    
    fem_words = [adword for adword in ad_text
        for word in fem_w
        if adword.startswith(word)]
    
    fem_w_count = len(fem_words)
    masc_w_count = len(masc_words)
    ...
    return {"result": result,
            "masculine_coded_words": masc_words,
            "feminine_coded_words": fem_words
            }
